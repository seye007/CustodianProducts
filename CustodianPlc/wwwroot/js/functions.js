	...
	/////////////////////////////////////////////
 	...
 	EXPRESS.page = {
		init: function () {
			...
			// BROWSER CHECK
			EXPRESS.page.browserCheck();

			// FITVIDS
			//EXPRESS.page.resizeVideos();

			// FITTEXT
			//EXPRESS.page.resizeHeadings();

			// HEADER SLIDER
			if (jQuery('body').hasClass('header-below-slider') && jQuery('.home-slider-wrap').length > 0) {
			EXPRESS.page.headerSlider();
			}

			// ONE PAGE NAV
			if (jQuery('#one-page-nav').length > 0) {
				EXPRESS.page.onePageNav();
			}

			// BACK TO TOP
			if (jQuery('#back-to-top').length > 0) {
				$window.scroll(function() {
					EXPRESS.page.backToTop();
				});
			}

			// EXPANDING ASSETS
			EXPRESS.page.expandingAssets();

			// RECENT POSTS
			if (jQuery('.recent-posts').length > 0) {
				EXPRESS.recentPosts.init();
			}

			// MOVE MODALS TO BOTTOM OF PAGE
			EXPRESS.page.moveModals();
			...
			// SMOOTH SCROLL LINKS
			EXPRESS.page.smoothScrollLinks();

			// PORTFOLIO STICKY SIDEBAR
			if (!isMobileAlt && jQuery('.sticky-details').length > 0) {
				EXPRESS.portfolio.stickyDetails();
			}
			...
			jQuery('.love-it').on('click', function() {
				EXPRESS.page.loveIt(jQuery(this));
				return false;
			});

			// ARTICLE SHARE
			EXPRESS.page.articleShare();

			// ARTICLE NAVIGATION
			if (body.hasClass('article-swipe') && body.hasClass('single-post') && jQuery('.post-pagination-wrap').length > 0) {
				EXPRESS.page.articleNavigation();
			}

			// ARTICLE WITH FULL WIDTH MEDIA TITLE
			if (jQuery('article').hasClass('single-post-fw-media-title')) {
				EXPRESS.page.postMediaTitle();
			}

			// RELATED POSTS
			if (jQuery('.related-items').length > 0) {
				EXPRESS.relatedPosts.init();
			}

			// LIGHTBOX
			EXPRESS.page.lightbox();

			// PAGE FADE OUT
			if (body.hasClass('page-transitions')) {
				EXPRESS.page.pageTransitions();
			}
		},
		load: function() {
			...
			if (jQuery('.fancy-heading').length > 0) {
				EXPRESS.page.fancyHeading();
			}

			// STICKY SIDEBAR
			if (!isMobileAlt && sfIncluded.hasClass('stickysidebars') && jQuery('.sidebar').length > 0) {
				EXPRESS.page.stickyWidget();
			}
			...
			setTimeout(function() {
				...
				if (urlHash.match('#')) {
				    ...
				    if ( jQuery('#' + hash).length > 0 ) {
				        EXPRESS.page.onePageNavGoTo('#' + hash);
				    }
				}
			}, 200);
		},
...
//			jQuery('.blog-items:not(.carousel-items),article.type-portfolio,article.type-post,article.type-team,.spb_video_widget,.infocus-item,.full-width-detail,#activity-stream').fitVids({ ignore: '.no-fv'});
		...
		resizeHeadings: function() {
			...
			EXPRESS.page.resizeHeadingsResize(h1FontSize, h2FontSize);
			$window.smartresize( function() {
				EXPRESS.page.resizeHeadingsResize(h1FontSize, h2FontSize);
			});
		},
		...
		onePageNav: function() {
			...
			if (pageSectionCount > 0) {
				...
				setTimeout(function() {
					...
					EXPRESS.page.onePageNavScroll(onePageNav);
					...
					jQuery('#one-page-nav ul li a').bind('click', function(e) {
						EXPRESS.page.onePageNavGoTo(jQuery(this).attr('href'));
						e.preventDefault();
					});
					...
					jQuery('#one-page-nav a.opn-up').bind('click', function(e) {
						...
						if (prevSection > 0) {
							EXPRESS.page.onePageNavGoTo(prevSectionHref);
						}
						e.preventDefault();
					});
					...
					jQuery('#one-page-nav a.opn-down').bind('click', function(e) {
						...
						if (nextSection <= pageSectionCount) {
							EXPRESS.page.onePageNavGoTo(nextSectionHref);
						}
						e.preventDefault();
					});
		...
		articleShare: function() {
			jQuery('.article-share').each(function() {
				...
				var config = {
				   	image: image,
				    ui: {
				        buttonText: articleShare.data('buttontext')
				    },
				};
		...
		stickyWidget: function() {
			...
			EXPRESS.page.initStickyWidget(stickyWidget, sidebar, offset);

			$window.smartresize( function() {
				jQuery('.inner-page-wrap').stickem().destroy();
				EXPRESS.page.resizeStickyWidget(stickyWidget, sidebar);
				EXPRESS.page.initStickyWidget(stickyWidget, sidebar, offset);
			});
		...
		pageTransitions: function() {
			jQuery('a').on('click', function(e) {
				...
				if (link.indexOf('#') === 0 && link.length > 1) {
					EXPRESS.isScrolling = true;
					EXPRESS.page.onePageNavGoTo(link);
					setTimeout(function() {
						EXPRESS.isScrolling = false;
					}, 1000);
					e.preventDefault();
		...
		init: function() {
			...
			jQuery('.super-search-go').on('click', function(e) {
				...
				var parentSearch = jQuery(this).parents('.sf-super-search'),
					filterURL = EXPRESS.superSearch.urlBuilder(parentSearch),
	...
	EXPRESS.header = {
		init: function() {
			...
			if (body.hasClass('sticky-header-enabled') && !body.hasClass('vertical-header')) {
				EXPRESS.header.stickyHeaderInit();
			}
			
			// STICKY TOP BAR
			if ( jQuery('.sticky-top-bar').length > 0 ) {
				EXPRESS.header.stickyTopBarInit();
			}
			...
			jQuery('a.overlay-menu-link').on('click', function(e) {
				e.preventDefault();
				EXPRESS.header.overlayMenuToggle();
			});
	...
	EXPRESS.woocommerce = {
		init: function() {
			...
			// QUANTITY FUNCTIONS
			EXPRESS.woocommerce.productQuantityAdjust();
		...
		load: function() {
			...
			if (jQuery('.woocommerce-shop-page').hasClass('full-width-shop') && !(IEVersion && IEVersion < 9)) {
				EXPRESS.woocommerce.fullWidthShop();
			}
				...
				variationSelect.on("change", function(){
					if (jQuery('#sf-included').hasClass('has-productzoom')) {
						...
						setTimeout(function() {
							...
							EXPRESS.woocommerce.productZoom(firstImage);
		...
		productZoom: function(zoomObject) {
			...
			$window.smartresize( function() {
				...
				EXPRESS.woocommerce.productZoom(currentImage);
			});
		},
		fullWidthShop: function() {
			...
			if (shopSidebar.length > 0) {
				...
				EXPRESS.woocommerce.fullWidthShopSetSidebarHeight();
				$window.smartresize( function() {
					EXPRESS.woocommerce.fullWidthShopSetSidebarHeight();
				});
				...
				EXPRESS.woocommerce.animateItems(shopItems);
	...
	EXPRESS.flexSlider = {
		init: function() {
			...
			if (jQuery('#product-img-slider').length > 0) {
				EXPRESS.flexSlider.productSlider();
			}
			...
			jQuery('.content-slider').each(function() {
				...
				slider.flexslider({
					...
					pauseOnHover: true,
					start: function() {
						EXPRESS.flexSlider.slideEqualHeights();
						$window.smartresize( function() {
							EXPRESS.flexSlider.slideEqualHeights();
						});
					}
			...
			jQuery('#product-img-slider').flexslider({
				...
				start: function(productSlider) {
					if (hasProductZoom) {
						if (productSlider.slides) {
							...
							EXPRESS.woocommerce.productZoom(currentImageHasSlides);
						} else {
							var currentImageNoSlides = jQuery('#product-img-slider').find('.product-slider-image');
							EXPRESS.woocommerce.productZoom(currentImageNoSlides);
						}
					}
				},
				before: function() {
				...
				after: function(productSlider) {
					if (hasProductZoom) {
						...
						EXPRESS.woocommerce.productZoom(currentSlideImage);
					}
				}
			});
		},
	...
	EXPRESS.portfolio = {
		init: function() {
			portfolioContainer.each(function() {
				...
				if (portfolioInstance.hasClass('masonry-items') && !(IEVersion && IEVersion < 9)) {
					EXPRESS.portfolio.masonrySetup(portfolioInstance);
				} else if (portfolioInstance.hasClass('multi-masonry-items') && !(IEVersion && IEVersion < 9)) {
					EXPRESS.portfolio.multiMasonrySetup(portfolioInstance);
				} else {
					EXPRESS.portfolio.standardSetup(portfolioInstance);
				}
			});

			// PORTFOLIO WINDOW RESIZE
			$window.smartresize( function() {
				EXPRESS.portfolio.windowResized();
			});
		...
		standardSetup: function(portfolioInstance) {
			portfolioInstance.imagesLoaded(function () {
				EXPRESS.portfolio.setItemHeight();
				EXPRESS.flexSlider.thumb();
			...
			portfolioInstance.appear(function() {
				EXPRESS.portfolio.animateItems(portfolioInstance);
			});
		},
		...
		multiMasonrySetup: function(portfolioInstance) {
			portfolioInstance.imagesLoaded(function () {
				EXPRESS.flexSlider.thumb();
				EXPRESS.portfolio.multiMasonrySizeFix();
				portfolioInstance.isotope({
					resizable: false,
					itemSelector : '.portfolio-item',
					layoutMode: 'packery',
		...
		portfolioShowcaseInit: function() {
			EXPRESS.flexSlider.thumb();
			EXPRESS.portfolio.portfolioShowcaseWrap();
			EXPRESS.portfolio.portfolioShowcaseItems();
			$window.smartresize( function() {
				EXPRESS.portfolio.portfolioShowcaseWrap();
				EXPRESS.portfolio.portfolioShowcaseItems();
			});
		...
		stickyDetails: function() {
			...
			EXPRESS.portfolio.stickyDetailsScroll();
			$window.scroll(function() {
				EXPRESS.portfolio.stickyDetailsScroll();
			});
		},
	...
	EXPRESS.blog = {
		init: function() {
			...
			blogItems.each(function() {
				...
				if (blogInstance.hasClass('blog-grid-items')) {
					EXPRESS.blog.blogGrid(blogInstance.find('.grid-items'));
				} else {
					EXPRESS.blog.blogLayout(blogInstance);
				}
			});

			// Blog Filtering
			EXPRESS.blog.blogFiltersInit();
		...
		blogLayout: function(blogInstance) {
			...
			if (blogType === "masonry" && blogInstance.hasClass('social-blog')) {
				...
				blogInstance.imagesLoaded(function () {
					...
					EXPRESS.blog.masonryInstagram(instagrams, blogInstance);
					}
				...
				blogInstance.appear(function() {
					EXPRESS.blog.animateItems(blogInstance);
				});
			}
		},
		...
		blogGrid: function(gridItems) {
			...
			gridItems.imagesLoaded(function () {
				...
				if (instagrams.length > 0) {
				EXPRESS.blog.blogGridInstagram(instagrams, gridItems);
				}
				gridItems.isotope('updateSortData').isotope();
				EXPRESS.blog.blogGridResize();
			}).animate({
				'opacity' : 1
			}, 800, 'easeOutExpo');
	...
	EXPRESS.galleries = {
		init: function() {
			galleriesContainer.each(function() {
				...
				if (galleriesInstance.hasClass('masonry-items')) {
					EXPRESS.galleries.masonrySetup(galleriesInstance);
				} else {
					EXPRESS.galleries.standardSetup(galleriesInstance);
				}
			});

			// PORTFOLIO WINDOW RESIZE
			$window.smartresize( function() {
				EXPRESS.galleries.windowResized();
			});
		...
		standardSetup: function(galleryInstance) {
			galleryInstance.imagesLoaded(function () {
				EXPRESS.galleries.setItemHeight();
			...
			galleryInstance.appear(function() {
				EXPRESS.galleries.animateItems(galleryInstance);
			});
		},
	...
	EXPRESS.gallery = {
		init: function() {
			...
			jQuery('.spb_gallery_widget').each(function() {
				if (jQuery(this).hasClass('gallery-masonry')) {
					EXPRESS.gallery.galleryMasonry(jQuery(this).find('.filterable-items'));
				} else if (jQuery(this).hasClass('gallery-slider')) {
					EXPRESS.gallery.gallerySlider(jQuery(this));
				}
			});
		},
		galleryMasonry: function(element) {
			...
			element.appear(function() {
				EXPRESS.gallery.animateItems(element);
			});
		},
				...
				carouselInstance.imagesLoaded(function () {
					...
					carouselInstance.owlCarousel({
						...
						afterInit: function() {
							...
							EXPRESS.parallax.init(true);
						}
					}).animate({
						'opacity': 1
					},800);

					EXPRESS.carouselWidgets.carouselMinHeight(carouselInstance);
					$window.smartresize( function() {
						EXPRESS.carouselWidgets.carouselMinHeight(carouselInstance);
					});
	...
	EXPRESS.widgets = {
		init: function() {
			...
			// LOAD WIDGETS
			EXPRESS.widgets.accordion();
			EXPRESS.widgets.toggle();
			EXPRESS.widgets.fullWidthVideo();
			EXPRESS.widgets.introAnimations();
			EXPRESS.widgets.iconBoxes();
			EXPRESS.widgets.countAssets();

			if (sfIncluded.hasClass('has-countdown')) {
			EXPRESS.widgets.countdownAssets();
			}

			if (sfIncluded.hasClass('has-imagebanner')) {
			EXPRESS.widgets.imageBanners();
			}
		...
		initSkillBars: function() {
			// SKILL BARS
			EXPRESS.widgets.animateSkillBars();
		...
		fullWidthVideo: function() {
			body.on('click', '.fw-video-link', function(){
				...
				EXPRESS.widgets.openFullWidthVideo(jQuery(this));
				}
				return false;
			});

			body.on('click', '.fw-video-close', function(){
				EXPRESS.widgets.closeFullWidthVideo();
			});
		},
		...
		init: function(reset) {
			...
			jQuery('.spb_parallax_asset').each(function(reset) {
				...
				if (parallaxAsset.hasClass('sf-parallax-video')) {
					...
					if (!isMobileAlt) {
						EXPRESS.parallax.parallaxVideoInit();
					} else {
					...
					jQuery(window).load(function() {
					    EXPRESS.parallax.parallaxVideoResize(parallaxAsset);
					});
					$window.smartresize( function() {
						EXPRESS.parallax.parallaxVideoResize(parallaxAsset);
					});
					...
					// Add resize handler
					setTimeout(function() {
						EXPRESS.parallax.windowImageResize(parallaxAsset);
					}, 500);
		...
		parallaxVideoInit: function() {
			jQuery('.spb_parallax_asset.sf-parallax-video').each(function() {
				...
				if (parallaxAsset.hasClass('parallax-window-height')) {
					...
					EXPRESS.parallax.scaleVideo(parallaxAsset);
				}
		...
		init: function() {
			jQuery('.spb_multilayer_parallax').each(function() {
				...
				mlParallaxAsset.imagesLoaded(function () {
					...
					EXPRESS.mlparallax.setContentLayerPos(mlParallaxAsset);
				...
				// Resize
				$window.smartresize( function() {
					EXPRESS.mlparallax.setAssetHeight(mlParallaxAsset);
				});
			});
		},
	...
	EXPRESS.map = {
		init:function() {
			...
			maps.each(function(i, element) {
				...
				EXPRESS.map.createMap( mapContainer, mapZoom, mapType, mapColor, mapSaturation, jQuery(this), mapCenterLat, mapCenterLng );
			});
		...
		addMarker: function(mapInstance, pinLogoURL, pinTitle, pinContent, pinLink, address, pinButtonText) {
			...
			EXPRESS.map.addWinContent(companyMarker, html, mapInstance);
		...
		createMap: function(mapContainer, mapZoom, mapType, mapColor, mapSaturation, targetMap, mapCenterLat, mapCenterLng) {
			...
			targetMap.parent().find('.pin_location').each(function(i, element) {
				...
				if (latitude === '' && longitude === '') {
					...
					EXPRESS.map.getLatLong(address, pinTitle, pinLogoURL ,function(location, lati,longi, pintit, pinimage ){
						...
						EXPRESS.map.addMarker(mapInstance, pinLogoURL, pinTitle, pinContent, pinLink, address, pinButtonText);
	...
	EXPRESS.mapDirectory = {
		init : function(searchTerm, locationFilter, categoryFilter) {
			...
			mapsDirectory.each(function(i, element) {
				...
				EXPRESS.mapDirectory.directoryMap( mapDirContainer, mapZoom, mapType, mapColor, mapSaturation, jQuery(this), searchTerm, locationFilter, categoryFilter);

			});
		},
		searchDirectory: function(){
			EXPRESS.mapDirectory.init(jQuery('#dir-search-value').val(), jQuery('.directory-location-option').val(), jQuery('.directory-category-option').val());
		},
		...
		addMarkerDir: function(mapInstance, pinLogoURL, pinTitle, pinContent, pinLink, address, pinButtonText,pinFeaturedImage, lat, lng){
			...
			infowindow = new google.maps.InfoWindow();
			EXPRESS.mapDirectory.addWinContent(companyMarker, html, mapInstance);
				...
				if (pincount > 0 && mapResults != 'list') {
					...
					jQuery.each(json.items, function(i, item) {
						...
						if ( mapResults != 'list') {
							...
							EXPRESS.mapDirectory.addMarkerDir(mapInstance, pinLogoURL, pinTitle, pinContent, pinLink, address, pinButtonText,pinThumbnail, latitude, longitude);
	...
	var $window = jQuery(window),
		...
		windowheight = EXPRESS.page.getViewportHeight(),
		...
		IEVersion = EXPRESS.page.checkIE(),
	...
	var onReady = {
		init: function(){
			...
			EXPRESS.woocommerce.init();
			}
			EXPRESS.woocommerce.cartWishlist();
			if (sfIncluded.hasClass('has-portfolio')) {
			EXPRESS.portfolio.init();
			}
			if (sfIncluded.hasClass('has-portfolio-showcase')) {
			EXPRESS.portfolio.portfolioShowcaseInit();
			}
			if (sfIncluded.hasClass('has-blog')) {
			EXPRESS.blog.init();
			}
			if (sfIncluded.hasClass('has-infscroll') && !isMobile) {
			EXPRESS.blog.infiniteScroll();
			}
			if (sfIncluded.hasClass('has-gallery')) {
			EXPRESS.gallery.init();
			}
			if (sfIncluded.hasClass('has-galleries')) {
			EXPRESS.galleries.init();
			}
			EXPRESS.widgets.init();
			if (sfIncluded.hasClass('has-team')) {
			EXPRESS.teamMembers.init();
			}
			if (sfIncluded.hasClass('has-carousel')) {
			EXPRESS.carouselWidgets.init();
			}
			if (sfIncluded.hasClass('has-parallax')) {
			EXPRESS.parallax.init();
			}
			if (sfIncluded.hasClass('has-ml-parallax')) {
			EXPRESS.mlparallax.init();
			} 
			EXPRESS.reloadFunctions.init();
		}
	};
	var onLoad = {
		init: function(){
			EXPRESS.flexSlider.init();
			if (sfIncluded.hasClass('has-parallax')) {
			EXPRESS.parallax.load();
			}
			if (sfIncluded.hasClass('has-products') || body.hasClass('woocommerce-cart') || body.hasClass('woocommerce-account')) {
			EXPRESS.woocommerce.load();
			}
			EXPRESS.page.load();
			if (body.hasClass('page-transitions')) {
			EXPRESS.page.fadePageIn();
			}
			if (sfIncluded.hasClass('has-chart')) {
			EXPRESS.widgets.charts();
			}
			EXPRESS.widgets.tabs();
			if (sfIncluded.hasClass('has-progress-bar')) {
			EXPRESS.widgets.initSkillBars();
			}
			if (sfIncluded.hasClass('has-map')) {
				EXPRESS.map.init();
				EXPRESS.mapDirectory.init('', '', '');
			...
			EXPRESS.reloadFunctions.load();
			EXPRESS.woocommerce.variations();
		}
	};
